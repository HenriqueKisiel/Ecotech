...
function cadastrarUsuario(req, res) {
  ...
  if (!usuario || usuario.trim() === '') erros.usuario = true;
  if (!senha || senha.trim() === '') erros.senha = true;
  ...
  if (Object.keys(erros).length > 0) {
    return res.render('usuarioAdicionar', {
      ...
      hasError: erros,
      script: `<script>
                swal("Erro ao adicionar!", "Preencha os campos obrigatórios!", {
                  icon: "error",
                  buttons: {
                    confirm: {
                      text: "OK",
  ...
  // Verifica se a pessoa física já tem usuário
  const verificaPessoaFisica = `SELECT * FROM usuario WHERE cd_pessoa_fisica = ?`;
  connection.query(verificaPessoaFisica, [nome], (err, resultsPessoa) => {
    if (err) {
      console.error('Erro ao verificar pessoa física:', err);
      return res.render('usuarioAdicionar', {
        usuario: req.body,
        script: `<script>
                  swal("Erro!", "Erro ao verificar pessoa física no banco de dados.", {
                    icon: "error",
                    buttons: {
                      confirm: {
                        text: "OK",
    ...
    if (resultsPessoa.length > 0) {
      return res.render('usuarioAdicionar', {
        usuario: req.body,
        script: `<script>
                  swal("Já possui usuário!", "Esta pessoa física já possui um usuário cadastrado.", {
                    icon: "warning",
                    buttons: {
                      confirm: {
                        text: "OK",
    ...
    // Verifica se já existe um nome de usuário igual
    const verificaNomeUsuario = `SELECT * FROM usuario WHERE nm_usuario = ?`;
    connection.query(verificaNomeUsuario, [usuario], (err, resultsUsuario) => {
      if (err) {
        console.error('Erro ao verificar nome de usuário:', err);
        return res.render('usuarioAdicionar', {
          usuario: req.body,
          script: `<script>
                    swal("Erro!", "Erro ao verificar nome de usuário.", {
                      icon: "error",
                      buttons: {
                        confirm: {
                          text: "OK",
      ...
      if (resultsUsuario.length > 0) {
        return res.render('usuarioAdicionar', {
          usuario: req.body,
          script: `<script>
                    swal("Nome de usuário já existe!", "Já existe um usuário cadastrado com este nome.", {
                      icon: "warning",
                      buttons: {
                        confirm: {
                          text: "OK",
      ...
      // Se passou nas validações, faz o insert
      const sql = `
